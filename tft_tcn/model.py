...
class TemporalFusionTransformer(BaseModelWithCovariates):
        ...
        * non-linear variable processing in variable selection network can be shared among decoder and encoder
        ...
        if not abletion:
            if self.paper:
                ...
                dec_params = 6
                layer_num = 4
                num_channels_enc = [enc_params for i in range(layer_num)]
                num_channels_dec = [dec_params for i in range(layer_num)]
                self.encoder = TemporalConvNet(enc_params , num_channels_enc, kernel_size=2, dropout=0.2)
                #self.decoder = TemporalConvNet(dec_params, num_channels_dec, kernel_size=2, dropout=0.2)
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.abletion:
            ...
            if self.paper:   # LSTM (1)
            ...
            else:            ### TCN, Alternative to LSTM (2)

                input_tcn = self.static_context_initial_hidden_lstm(static_embedding).expand(
                    self.hparams.lstm_layers, -1, -1
                )
                # run local encoder
                ...
                # run local decoder
                decoder_output = embeddings_varying_decoder
